...
import { CheckCircle2, Download, ShieldCheck, Clock, ChevronDown, ChevronUp } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen relative font-sans  bg-gray-50">
      <nav className="sticky border-b-1  border-gray-100 mb-2 md:mb-0 top-0 z-50 px-4 bg-gray-100 md:bg-gray-50/80 backdrop-blur-md py-4 flex justify-between items-center max-w-6xl mx-auto ">
        ...
        <button className="bg-green-700 text-white px-8 py-2 rounded-lg font-semibold hover:bg-green-800 transition">
          Get App
        </button>
      </nav>
      ...
      <main className="max-w-6xl mt-4 md:mt-0 mx-auto px-4 py-2 grid lg:grid-cols-2 gap-8 items-center">
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} >
          ...
          <button className="bg-green-700 text-white px-10 py-4 rounded-xl flex items-center gap-3 text-lg font-bold hover:shadow-xl hover:bg-green-800 transition w-full md:w-auto justify-center">
        ...
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="flex justify-center items-center md:w-[80%]  lg:justify-end">
          <img
            src={img}
            alt="App interface"
            className="h-[25rem] md:h-[30rem] w-auto "
          />
        </motion.div>
      </main>
      ...
      <section className="py-20 px-6 max-w-4xl mx-auto">
        ...
        <div className="space-y-4">
          <FAQItem
            ...
            answer="This platform is designed specifically for civil servants to manage their daily attendance. By requiring a morning check-in and an evening sign-out, it ensures that staff hours are accurately captured, promoting punctuality and transparency across all departments."
          />
          <FAQItem
            question="How does the sign-in/sign-out process work?"
            answer="Upon arrival, you use the app to perform a quick facial recognition check-in. The process is repeated for your evening sign-out. This provides a reliable, automated record that eliminates manual paperwork and potential errors."
          />
          <FAQItem
            question="Is my attendance record permanently saved?"
            answer="Yes. The system maintains a secure, digital history of your attendance. This allows administrators to pull accurate reports on a monthly or weekly basis, ensuring all records are kept consistent and verifiable."
          />
        </div>
      </section>
      {/* Footer */}
      <footer className="bg-gray-900 text-white py-12 px-6 mt-12">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-center gap-6">
          ...
          <div className="text-sm text-gray-500">
            © 2026 Abia State Government. All rights reserved.
          </div>
        </div>
      </footer>
